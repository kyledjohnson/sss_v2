# SeventeenSeventySix CI workers

This folder contains the Ansible playbooks for configuring a fresh OS
installation for use as a Buildbot worker in SeventeenSeventySix's CI.
...
- Start a basic EC2 instance using the template AMI for the target OS.
  - Choose the smallest instance size, it won't be used for building SeventeenSeventySix.
...
- Create `inventory/hosts` containing the following:
    ...
    [seventeenseventysix-ci-worker-unix]
