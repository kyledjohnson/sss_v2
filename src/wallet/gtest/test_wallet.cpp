...
CWalletTx GetValidReceive(const libseventeenseventysix::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libseventeenseventysix::Note GetNote(const libseventeenseventysix::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libseventeenseventysix::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libseventeenseventysix::SpendingKey::random();
    auto sk2 = libseventeenseventysix::SpendingKey::random();
