...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"seventeenseventysixprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"seventeenseventysixprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid SeventeenSeventySix address or script");
    }
...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys)
{
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid SeventeenSeventySix spending key
        if (fImportZKeys) {
            try {
                CZCSpendingKey spendingkey(vstr[0]);
                libseventeenseventysix::SpendingKey key = spendingkey.Get();
                libseventeenseventysix::PaymentAddress addr = key.address();
                ...
                // Not a valid spending key, so carry on and see if it's a SeventeenSeventySix style address.
            }
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"seventeenseventysixaddress\"\n"
            "\nReveals the private key corresponding to 'seventeenseventysixaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"seventeenseventysixaddress\"   (string, required) The seventeenseventysix address for the private key\n"
            "\nResult:\n"
...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename, saved in folder set by seventeenseventysixd -exportdir option\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the seventeenseventysixd -exportdir option has been set");
    }
    ...
    // produce output
    file << strprintf("# Wallet dump created by SeventeenSeventySix %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
