...
#endif // ENABLE_WALLET
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SeventeenSeventySixMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("SeventeenSeventySixMiner: ProcessNewBlock, block not accepted");
...
#endif
{
    LogPrintf("SeventeenSeventySixMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("seventeenseventysix-miner");
    const CChainParams& chainparams = Params();
    ...
    try {
        while (true) {
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in SeventeenSeventySixMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in SeventeenSeventySixMiner: Invalid -mineraddress\n");
                }
                return;
            }
            ...
            LogPrintf("Running SeventeenSeventySixMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true) {
                ...
                std::function<bool(std::vector<unsigned char>)> validBlock =
                    ...
                    LogPrintf("SeventeenSeventySixMiner:\n");
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("SeventeenSeventySixMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("SeventeenSeventySixMiner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
