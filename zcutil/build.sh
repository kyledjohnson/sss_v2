...
$0 [ --enable-lcov || --disable-tests ] [ --disable-mining ] [ --disable-rust ] [ --enable-proton ] [ --disable-libs ] [ MAKEARGS... ]
  Build SeventeenSeventySix and most of its transitive dependencies from
  source. MAKEARGS are applied to both dependencies and SeventeenSeventySix itself.

  If --enable-lcov is passed, SeventeenSeventySix is configured to add coverage
  instrumentation, thus enabling "make cov" to work.
  If --disable-tests is passed instead, the SeventeenSeventySix tests are not built.

  If --disable-mining is passed, SeventeenSeventySix is configured to not build any mining
  code. It must be passed after the test arguments, if present.

  If --disable-rust is passed, SeventeenSeventySix is configured to not build any Rust language
  assets. It must be passed after test/mining arguments, if present.

  If --enable-proton is passed, SeventeenSeventySix is configured to build the Apache Qpid Proton
  library required for AMQP support. This library is not built by default.
  It must be passed after the test/mining/Rust arguments, if present.

  If --disable-libs is passed, SeventeenSeventySix is configured to not build any libraries like
  'libseventeenseventysixconsensus'.
EOF
    exit 0
fi
