...
def main(args=sys.argv[1:]):
    """
    Perform the final SeventeenSeventySix release process up to the git tag.
    """
...
def update_debian_changelog(release):
    ...
    os.environ['DEBFULLNAME'] = 'SeventeenSeventySix Company'
    sh_log(
        'debchange',
...
def patch_README(release, releaseprev):
    with PathPatcher('README.md') as (inf, outf):
        ...
        assert firstline == 'SeventeenSeventySix {}\n'.format(releaseprev.novtext), \
            repr(firstline)

        outf.write('SeventeenSeventySix {}\n'.format(release.novtext))
        outf.write(inf.read())
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "seventeenseventysix-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "seventeenseventysix-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './seventeenseventysix-make-release.log'
    ...
    logging.info('seventeenseventysix make-release.py debug log: %r', logname)
